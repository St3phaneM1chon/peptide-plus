                  ...
                  {config.showLoyaltySection !== false && (
                    <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-200">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div>
                          <p className="text-sm text-indigo-700">{t(`${prefix}.tier`)}</p>
                          <StatusBadge variant={tierVariants[displayItem.loyaltyTier] || 'neutral'}>
                            {displayItem.loyaltyTier}
                          </StatusBadge>
                        </div>
                        <div>
                          <p className="text-sm text-indigo-700">{t(`${prefix}.currentPoints`)}</p>
                          <p className="text-2xl font-bold text-indigo-900">{displayItem.loyaltyPoints.toLocaleString(locale)}</p>
                        </div>
                        <div>
                          <p className="text-sm text-indigo-700">{t(`${prefix}.lifetimePoints`)}</p>
                          <p className="text-2xl font-bold text-indigo-900">{displayItem.lifetimePoints.toLocaleString(locale)}</p>
                        </div>
                      </div>
