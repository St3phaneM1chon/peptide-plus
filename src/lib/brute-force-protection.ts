...
async function getRedisRecord(key: string): Promise<{ attempts: number; firstAttempt: number; lockedUntil: number | null } | null> {
  try {
    ...
    logger.error('Redis get record failed', { error: error instanceof Error ? error.message : String(error) });
    return null;
  }
}
...
async function setRedisRecord(key: string, record: { attempts: number; firstAttempt: number; lockedUntil: number | null }): Promise<boolean> {
  try {
    ...
    logger.error('Redis set record failed', { error: error instanceof Error ? error.message : String(error) });
    return false;
  }
}
...
async function deleteRedisRecord(key: string): Promise<void> {
  try {
    ...
    logger.error('Redis delete record failed', { error: error instanceof Error ? error.message : String(error) });
  }
}
