    ...
    async jwt({ token, user, account, trigger, session }) {
      try {
        ...
        if ((trigger === 'update' || shouldRefreshRole) && token.id) {
          ...
          // AUTH-F7 FIX: Require server-signed proof for mfaVerified update.
          // The /api/auth/mfa/verify route generates an HMAC proof after TOTP verification.
          // Client passes this proof in update({ mfaVerified: true, mfaProof: '...' }).
          if (
            trigger === 'update' &&
            ...
            // Verify the HMAC proof (generated by MFA verify endpoint)
            const crypto = await import('crypto');
            const secret = process.env.NEXTAUTH_SECRET || '';
            const expectedProof = crypto.createHmac('sha256', secret)
              .update(`mfa-verified:${token.sub}`)
              .digest('hex');
            if (session.mfaProof === expectedProof) {
              token.mfaVerified = true;
            } else {
              logger.warn('Invalid MFA proof in session update', { userId: token.sub });
            }
          }
        }
