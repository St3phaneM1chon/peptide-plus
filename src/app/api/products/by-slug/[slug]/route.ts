...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // A7-P0-002: Redis cache for product-by-slug (QuickView)
    const cacheKey = `${CacheKeys.products.bySlug(slug)}:locale=${locale}`;

    const transformedProduct = await cacheGetOrSet(cacheKey, async () => {
