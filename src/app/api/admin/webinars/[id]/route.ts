  ...
  try {
    ...
    if (webinar.tags) {
      try {
        ...
        logger.error('[AdminWebinarById] Failed to parse webinar tags as JSON', { error: error instanceof Error ? error.message : String(error) });
...
export const PATCH = withAdminGuard(async (request: NextRequest, { session, params }) => {
  try {
    ...
    }).catch((err) => { logger.error('[admin/webinars/id] Non-blocking operation failed:', err); });
