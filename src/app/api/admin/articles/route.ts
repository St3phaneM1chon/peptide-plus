    ...
    const enrichedArticles = articles.map(a => {
      ...
      if (a.tags) {
        try {
          ...
          logger.error('[AdminArticles] Failed to parse article tags as JSON', { error: error instanceof Error ? error.message : String(error) });
...
export const POST = withAdminGuard(async (request, { session }) => {
  try {
    ...
    }).catch((err) => { logger.error('[admin/articles] Non-blocking operation failed:', err); });
