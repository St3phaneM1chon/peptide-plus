 ...
 * GET /api/feeds/tiktok — JSON product feed
 *
 * Reads all active products and formats per TikTok Shop catalog spec.
...
import { generateTikTokFeed } from '@/lib/social-commerce';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = await generateTikTokFeed({
      baseUrl,
      storeName,
      currency,
    });
    ...
    logger.error('[TikTokFeed] Failed to generate feed', {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      { error: 'Failed to generate TikTok Shop feed' },
      { status: 500 }
    );
  }
}
