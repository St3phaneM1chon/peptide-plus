...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/lms/rubrics`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/lms/rubrics', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
  ...
  const columns: Column<any>[] = [
    { key: 'rubrique', header: 'Rubrique', render: (row: any) => String(row.name) },
    { key: 'score_max', header: 'Score max', render: (row: any) => String(row.maxScore ?? 100) },
  ];

  return (
    <div className="space-y-6">
      <PageHeader title="Grilles d'evaluation" subtitle=""
      ...
      {loading ? (
        ...
        <EmptyState icon={ClipboardList} title={t('common.noData')} description={t('common.addToStart')} />
      ) : (
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Ajouter">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FormField label="Score max"><Input type="number" value={form.maxScore || ""} onChange={(e) => setForm((f: any) => ({ ...f, maxScore: e.target.value }))} /></FormField>
