...
export default function CallCenterKPIsClient() {
  const { t } = useI18n();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      else toast.error(t('admin.crm.fetchError'));
    } catch {
      toast.error(t('admin.crm.fetchError'));
    } finally {
      setLoading(false);
    }
  }, [period, t]);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {t('admin.crm.callCenterKpis')}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {t('admin.crm.callCenterKpisDesc')}
          </p>
        </div>
