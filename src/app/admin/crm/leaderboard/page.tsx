  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            {t('admin.crm.leaderboard')}
          </h1>
          <p className="text-sm text-gray-500 mt-1">{t('admin.crm.topAgents')}</p>
        </div>
        <div className="flex gap-2">
          {(['today', 'week', 'month'] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              ...
              {p === 'today' ? (t('common.today')) : p === 'week' ? (t('common.thisWeek')) : (t('common.thisMonth'))}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {([
          { key: 'conversions' as const, label: t('admin.crm.conversions'), icon: <TrendingUp className="h-4 w-4" /> },
          { key: 'calls' as const, label: t('admin.crm.calls'), icon: <Phone className="h-4 w-4" /> },
          { key: 'revenue' as const, label: t('admin.crm.revenue'), icon: <DollarSign className="h-4 w-4" /> },
        ]).map(m => (
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-16">
          ...
          <p>{t('admin.crm.noPerformanceData')}</p>
        </div>
      ) : (
            ...
            <div key={agent.id} className="flex items-center gap-4 bg-white border rounded-lg px-4 py-3 hover:bg-gray-50">
              ...
              <div className="flex gap-6 text-sm">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-400">{t('admin.crm.calls')}</p>
                </div>
                <div className="text-center">
                  <p className="font-bold text-purple-600">{agent.conversions}</p>
                  <p className="text-xs text-gray-400">{t('admin.crm.conv')}</p>
                </div>
                <div className="text-center">
                  <p className="font-bold text-emerald-600">{formatCurrency(agent.revenue)}</p>
                  <p className="text-xs text-gray-400">{t('admin.crm.revenue')}</p>
                </div>
                <div className="text-center">
                  <p className="font-bold text-indigo-600">{agent.contactRate}%</p>
                  <p className="text-xs text-gray-400">{t('admin.crm.contact')}</p>
                </div>
              </div>
            </div>
          ))}
