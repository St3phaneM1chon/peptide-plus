  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="flex gap-2">
          {(['today', 'week', 'month'] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              className={`px-3 py-1.5 text-sm rounded-md ${period === p ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        <SummaryCard icon={<Phone className="h-5 w-5 text-indigo-500" />} label="Total Calls" value={totals.calls.toString()} />
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {([
          ...
          <button key={s.key} onClick={() => setSortBy(s.key)}
            className={`px-2 py-1 text-xs rounded ${sortBy === s.key ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-500'}`}>
            {s.label}
          </button>
        ))}
      </div>
