...
const STATUS_COLORS: Record<string, string> = {
  OPEN: 'bg-indigo-100 text-indigo-700', PENDING: 'bg-yellow-100 text-yellow-700',
  RESOLVED: 'bg-green-100 text-green-700', CLOSED: 'bg-gray-100 text-gray-600',
};

const TEMP_COLORS: Record<string, string> = {
  HOT: 'text-red-600 bg-red-50', WARM: 'text-orange-600 bg-orange-50', COLD: 'text-indigo-600 bg-indigo-50',
};
    ...
    <div className="h-[calc(100vh-64px)] flex">
      ...
      <div className="w-80 border-r bg-[var(--k-glass-thin)] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto divide-y">
          {loading ? (
            <div className="p-4 text-center text-gray-400"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-indigo-500 mx-auto" /></div>
          ) : conversations.length === 0 ? (
            <div className="p-4 text-center text-gray-400 text-sm">{t('admin.crm.noConversations')}</div>
          ) : conversations.map(conv => {
            const ChannelIcon = CHANNEL_ICONS[conv.channel] || MessageSquare;
            const isSelected = selected?.id === conv.id;
            return (
              <button key={conv.id} onClick={() => selectConversation(conv)}
                className={`w-full text-start p-3 hover:bg-gray-50 transition-colors ${isSelected ? 'bg-indigo-50 border-s-2 border-indigo-500' : ''}`}>
        ...
        {!selected ? (
          <div className="flex-1 flex items-center justify-center text-gray-400">
            <div className="text-center">
              ...
              <p>{t('admin.crm.selectConversation')}</p>
            </div>
          </div>
        ) : (
          <>
              ...
              <div className="flex items-center gap-2">
                {slaRemaining !== null && slaRemaining <= 30 && (
                  <span className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${slaRemaining <= 0 ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    ...
                    {slaRemaining <= 0 ? t('admin.crm.slaBreached') : `${slaRemaining}m`}
                  </span>
                )}
                {selected.status !== 'RESOLVED' && (
                  <button onClick={() => updateStatus('RESOLVED')} className="flex items-center gap-1 px-3 py-1.5 text-xs bg-green-50 text-green-700 rounded-md hover:bg-green-100">
                    <CheckCircle2 className="h-3.5 w-3.5" /> {t('admin.crm.resolve')}
                  </button>
                )}
                {selected.status === 'RESOLVED' && (
                ...
                {selected.status === 'CLOSED' && (
                  <button onClick={() => updateStatus('OPEN')} className="flex items-center gap-1 px-3 py-1.5 text-xs bg-indigo-50 text-indigo-700 rounded-md hover:bg-indigo-100">
                    {t('admin.crm.reopen')}
                  </button>
                )}
              </div>
            ...
            <div className="bg-[var(--k-glass-thin)] border-t p-3">
              <div className="flex gap-2">
                <textarea
                  ...
                  placeholder={t('admin.crm.typeReply')}
                  rows={2}
                  className="flex-1 text-sm border rounded-md px-3 py-2 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendReply(); } }}
                />
                <button onClick={sendReply} disabled={sending || !replyText.trim()}
                  className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 self-end">
        ...
        <div className="w-72 border-l bg-[var(--k-glass-thin)] overflow-y-auto">
          ...
          {selected.lead && (
            <div className="p-4 border-b">
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('admin.crm.leadInfo')}</h4>
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-gray-50 rounded-lg p-2 text-center">
                  ...
                  <p className="text-xs text-gray-500">{t('admin.crm.score')}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-2 text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-1">{t('admin.crm.temperature')}</p>
                </div>
              </div>
              <div className="mt-2 space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-500 flex items-center gap-1"><Activity className="h-3 w-3" /> {t('admin.crm.status')}</span>
                  <span className="font-medium text-gray-700">{selected.lead.status}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-500 flex items-center gap-1"><Hash className="h-3 w-3" /> {t('admin.crm.source')}</span>
                  <span className="font-medium text-gray-700">{selected.lead.source}</span>
                </div>
                {selected.lead.lastContactedAt && (
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-500 flex items-center gap-1"><Calendar className="h-3 w-3" /> {t('admin.crm.lastContact')}</span>
              ...
              <a href={`/admin/crm/leads/${selected.lead.id}`} className="mt-2 text-xs text-indigo-600 hover:underline flex items-center gap-1">
                <ExternalLink className="h-3 w-3" /> {t('admin.crm.viewLeadProfile')}
              </a>
            </div>
          )}
          ...
          {selected.lead?.deals && selected.lead.deals.length > 0 && (
            <div className="p-4 border-b">
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('admin.crm.activeDeals')}</h4>
          ...
          <div className="p-4">
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('admin.crm.conversationDetails')}</h4>
            <div className="space-y-1.5 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-gray-500">{t('admin.crm.channel')}</span>
                <span className="font-medium text-gray-700">{selected.channel}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-500">{t('admin.crm.assignedTo')}</span>
                <span className="font-medium text-gray-700">{selected.assignedTo?.name || selected.assignedTo?.email || t('admin.crm.unassigned')}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-500">{t('admin.crm.messages')}</span>
                <span className="font-medium text-gray-700">{selected.messages?.length || 0}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-500">{t('admin.crm.created')}</span>
