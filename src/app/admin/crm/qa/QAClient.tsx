...
function CreateFormModal({ onClose, onCreated }: CreateFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      toast.error(t('admin.crm.qa.nameRequired'));
      return;
    }
    const validCriteria = criteria.filter((c) => c.name.trim());
    if (validCriteria.length === 0) {
      toast.error(t('admin.crm.qa.criteriaRequired'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('admin.crm.qa.formCreated'));
      onCreated(json.data);
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            ...
            <input
              ...
              className={inputCls}
              placeholder={t('admin.crm.qa.formNamePlaceholder')}
            />
          </div>

          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">
              {t('admin.crm.qa.description')}
            </label>
            <textarea
              value={description}
              ...
              rows={2}
              placeholder={t('admin.crm.qa.descriptionPlaceholder')}
            />
          </div>
              ...
              {criteria.map((criterion, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
                  <input
                    ...
                    className="flex-1 border border-gray-200 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    placeholder={t('admin.crm.qa.criterionName')}
                  />
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      className="w-14 border border-gray-200 rounded px-2 py-1.5 text-sm text-center focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                  </div>
                  <div className="flex items-center gap-1">
...
function ScoreCallModal({ forms, onClose, onScored }: ScoreCallModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!agentId.trim()) {
      toast.error(t('admin.crm.qa.agentRequired'));
      return;
    }
    if (!selectedFormId) {
      toast.error(t('admin.crm.qa.formRequired'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('admin.crm.qa.scoreCreated'));
      onScored(json.data);
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className={inputCls}
                placeholder={t('admin.crm.qa.optional')}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className={inputCls + ' resize-none'}
              rows={3}
              placeholder={t('admin.crm.qa.feedbackPlaceholder')}
            />
          </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            {t('admin.crm.qa.title')}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {t('admin.crm.qa.subtitle')}
          </p>
        </div>
          ...
          {formsLoading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-sm text-gray-500 mt-1 mb-4">
                {t('admin.crm.qa.noFormsDesc')}
              </p>
              <button
                onClick={() => setShowCreateForm(true)}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg hover:bg-indigo-700"
              >
                <Plus className="h-4 w-4" />
                {t('admin.crm.qa.createFirst')}
              </button>
            </div>
          ) : (
              ...
              {forms.map((form) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400 pt-3 border-t border-gray-100">
                    <span>{form.criteria.length} {t('admin.crm.qa.criteria')}</span>
                    <span>{form._count.scores} {t('admin.crm.qa.evaluations')}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {showFilters && (
              <div className="mt-2 bg-white rounded-lg border border-gray-200 p-4 flex gap-4">
                <div className="flex-1">
                  ...
                  >
                    <option value="">{t('admin.crm.qa.allForms')}</option>
                    {forms.map((f) => (
                      <option key={f.id} value={f.id}>{f.name}</option>
                    ))}
                  </select>
                ...
                >
                  ...
                  {score.feedback && (
                    <div className="text-xs text-gray-600 bg-yellow-50 rounded-lg px-3 py-2 border border-yellow-100">
                      <span className="font-medium">{t('admin.crm.qa.feedback')}:</span> {score.feedback}
                    </div>
                  )}
                </div>
              ))}
        ...
        <div>
          ...
          {calibrationSessions.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-sm text-gray-500 mt-1 mb-4">
                {t('admin.crm.qa.noCalibrationsDesc')}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="font-semibold text-gray-900 flex items-center gap-2">
                          ...
                          {t('admin.crm.qa.calibrationSession')}
                        </h3>
                    ...
                    {chartData.length >= 2 && (
                      <div className="bg-gray-50 rounded-lg p-4">
                        <h4 className="text-xs font-medium text-gray-600 mb-3">
                          {t('admin.crm.qa.scoreComparison')}
                        </h4>
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                  <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    {t('admin.crm.qa.setupCalibration')}
                  </h3>
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        toast.success(t('admin.crm.qa.calibrationCreated'));
                      }}
                      className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-purple-600 rounded-lg hover:bg-purple-700"
                    >
                      <Play className="h-4 w-4" />
                      {t('admin.crm.qa.startCalibration')}
                    </button>
                  </div>
                </div>
              </div>
