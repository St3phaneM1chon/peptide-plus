...
export default function CompliancePage() {
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    { key: 'overview', label: t('admin.crm.overview'), icon: <Shield className="h-4 w-4" /> },
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {tab === 'overview' && stats && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <StatCard icon={<FileText className="h-5 w-5 text-indigo-500" />} label="Consent Records" value={stats.consentRecords} />
      ...
      {tab === 'rules' && (
        <div className="space-y-4">
          <button onClick={() => setShowAddRule(!showAddRule)} className="flex items-center gap-2 px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
