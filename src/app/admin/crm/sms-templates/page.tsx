...
export default function SmsTemplatesPage() {
  ...
  const saveTemplate = async () => {
    if (!form.name.trim() || !form.body.trim()) { toast.error(t('admin.crm.nameBodyRequired')); return; }
    setSaving(true);
    try {
      ...
      const json = await res.json();
      if (json.success) {
        toast.success(editingId ? (t('admin.crm.templateUpdated')) : (t('admin.crm.templateCreated')));
        setShowCreate(false);
        setEditingId(null);
  ...
  const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text).then(
      () => toast.success(t('admin.crm.copied')),
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('admin.crm.smsTemplates')}</h1>
            <p className="text-sm text-gray-500">{t('admin.crm.smsTemplatesDesc')}</p>
          </div>
        </div>
        <button onClick={() => { setShowCreate(true); setEditingId(null); setForm({ name: '', body: '', variables: '' }); }}
          ...
          <Plus className="h-4 w-4" /> {t('admin.crm.newTemplate')}
        </button>
      </div>
      ...
      <div className="flex gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={t('admin.crm.searchTemplates')}
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-gray-400">{charCount} {t('admin.crm.chars')} / {segCount} {t('admin.crm.segments')}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-0.5">
                      <button onClick={() => setPreviewId(isPreview ? null : tpl.id)} className="p-1.5 hover:bg-indigo-50 rounded text-indigo-500" title={t('admin.crm.preview')}>
                        <Eye className="h-4 w-4" />
                      </button>
                      <button onClick={() => copyToClipboard(tpl.body)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400" title={t('admin.crm.copy')}>
                        <Copy className="h-4 w-4" />
                      </button>
                      <button onClick={() => startEdit(tpl)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400" title={t('common.edit')}>
                      ...
                      <button onClick={() => deleteTemplate(tpl.id)} className="p-1.5 hover:bg-red-50 rounded text-red-400" title={t('common.delete')}>
                    ...
                    {isPreview ? (
                      <div>
                        <div className="flex items-center gap-1 mb-1 text-xs text-indigo-600 font-medium">
                          <Eye className="h-3 w-3" /> {t('admin.crm.livePreview')}
                        </div>
                        <p>{previewText}</p>
                      </div>
                    ) : (
                  ...
                  {tpl.variables.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {tpl.variables.map(v => (
                        <span key={v} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-full flex items-center gap-0.5">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 focus:outline-none" autoFocus placeholder="e.g. Welcome message" />
              </div>

              {/* Body + Preview side by side */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">{t('admin.crm.messageBody')} *</label>
                  <textarea value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))}
                    className="w-full border rounded-md px-3 py-2 text-sm font-mono resize-none focus:ring-2 focus:ring-indigo-400 focus:outline-none" rows={6}
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <p className={`text-xs ${form.body.length > 160 ? 'text-orange-600' : 'text-gray-400'}`}>
                      {form.body.length}/160 {t('admin.crm.chars')} ({segments} {t('admin.crm.segments')})
                    </p>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 flex items-center gap-1">
                    <Eye className="h-3.5 w-3.5 text-indigo-500" /> {t('admin.crm.livePreview')}
                  </label>
                  <div className="bg-gray-50 border rounded-md px-3 py-2 text-sm min-h-[150px] whitespace-pre-wrap">
                    {formPreview || <span className="text-gray-400 italic">{t('admin.crm.typeToPreview')}</span>}
                  </div>
                  {form.body.length > 160 && (
                    <p className="text-xs text-orange-600 mt-1">{t('admin.crm.multipleSegments')}</p>
                  )}
                </div>
              </div>
              ...
              {autoDetectedVars.length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1 flex items-center gap-1">
                    <Variable className="h-3 w-3" /> {t('admin.crm.detectedVariables')}
                  </label>
                  <div className="flex flex-wrap gap-1">
                    {autoDetectedVars.map(v => (
                      <span key={v} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-full">{`{{${v}}}`}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {autoDetectedVars.length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.crm.customPreviewValues')}</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">{t('admin.crm.variablesOverride')}</label>
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t bg-gray-50 rounded-b-xl">
              ...
              <button onClick={saveTemplate} disabled={saving || !form.name.trim() || !form.body.trim()}
                className="px-4 py-2 text-sm text-white bg-indigo-600 rounded-md hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-1">
                {saving ? '...' : <><Check className="h-4 w-4" /> {editingId ? (t('common.save')) : (t('common.create'))}</>}
              </button>
            </div>
          </div>
        </div>
