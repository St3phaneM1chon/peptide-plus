  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--k-text-primary)]">{t('admin.crm.deals')}</h1>
          <p className="text-sm text-[var(--k-text-tertiary)] mt-1">{total} {t('admin.crm.deals')}</p>
        </div>
        <div className="flex items-center gap-2">
          {/* View toggle */}
          <div className="flex items-center border border-[var(--k-border-default)] rounded-md overflow-hidden">
            <button
              className="flex items-center gap-1 px-2.5 py-2 text-sm bg-[#6366f1]/10 text-[#818cf8] border-r border-[var(--k-border-default)]"
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-2 text-sm text-[var(--k-text-tertiary)] hover:bg-[var(--k-glass-thin)]"
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-3 mb-4">
          <div className="bg-[var(--k-glass-thin)] backdrop-blur-sm rounded-lg border border-[var(--k-border-subtle)] px-4 py-3">
            <div className="flex items-center gap-2 text-xs text-[var(--k-text-tertiary)] mb-1">
          ...
          <div className="bg-[var(--k-glass-thin)] backdrop-blur-sm rounded-lg border border-[var(--k-border-subtle)] px-4 py-3">
            ...
            <p className="text-lg font-bold text-[#818cf8]">
          ...
          <div className="bg-[var(--k-glass-thin)] backdrop-blur-sm rounded-lg border border-[var(--k-border-subtle)] px-4 py-3">
            ...
            <div className="flex items-center gap-2 text-[10px] text-[var(--k-text-muted)] mt-0.5">
          ...
          <div className="bg-[var(--k-glass-thin)] backdrop-blur-sm rounded-lg border border-[var(--k-border-subtle)] px-4 py-3">
            ...
            <p className="text-lg font-bold text-[var(--k-text-primary)]">{stats.openCount}</p>
            <p className="text-[10px] text-[var(--k-text-muted)] mt-0.5">{stats.totalDeals} {t('common.total')}</p>
          </div>
        </div>
      )}
          ...
          <table className="w-full">
            <thead className="bg-[var(--k-glass-thin)] border-b border-[var(--k-border-subtle)]">
              <tr>
                ...
                <th className="px-4 py-3 text-start text-xs font-medium text-[var(--k-text-tertiary)] uppercase">{t('admin.crm.dealTitle')}</th>
                <th className="px-4 py-3 text-end text-xs font-medium text-[var(--k-text-tertiary)] uppercase">{t('admin.crm.dealValue')}</th>
                <th className="px-4 py-3 text-start text-xs font-medium text-[var(--k-text-tertiary)] uppercase">{t('admin.crm.status')}</th>
                <th className="px-4 py-3 text-start text-xs font-medium text-[var(--k-text-tertiary)] uppercase">{t('admin.crm.contact')}</th>
                <th className="px-4 py-3 text-start text-xs font-medium text-[var(--k-text-tertiary)] uppercase">{t('admin.crm.assignedTo')}</th>
                <th className="px-4 py-3 text-start text-xs font-medium text-[var(--k-text-tertiary)] uppercase">{t('admin.crm.expectedCloseDate')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--k-border-subtle)]">
              {loading ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-[var(--k-text-muted)]">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-indigo-500 mx-auto" />
                </td></tr>
              ) : deals.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-16 text-center">
                  <DollarSign className="h-10 w-10 text-[var(--k-text-muted)] mx-auto mb-3" />
                  <p className="text-sm font-medium text-[var(--k-text-tertiary)]">{t('admin.crm.noDeals')}</p>
                  <p className="text-xs text-[var(--k-text-muted)] mt-1">{t('admin.crm.noDealsDescription')}</p>
                </td></tr>
              ) : deals.map((deal) => (
                <tr
                  key={deal.id}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium text-[var(--k-text-primary)]">{deal.title}</div>
                    {deal.tags.length > 0 && (
                      <div className="flex gap-1 mt-1">
                        {deal.tags.slice(0, 3).map(tag => (
                          <span key={tag} className="inline-flex px-1.5 py-0.5 text-[10px] bg-[var(--k-glass-thin)] text-[var(--k-text-tertiary)] rounded">{tag}</span>
                        ))}
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3 text-end">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span className="text-sm font-semibold text-[var(--k-text-primary)]">{formatCurrency(deal.value, deal.currency)}</span>
                    </div>
                    <div className="text-[10px] text-[var(--k-text-muted)] text-end mt-0.5">
                  ...
                  <td className="px-4 py-3 text-sm text-[var(--k-text-secondary)]">
