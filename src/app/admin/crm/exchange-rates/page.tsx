...
export default function ExchangeRatesPage() {
  ...
  const syncRates = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) { toast.success(t('admin.crm.ratesSynced')); fetchRates(); }
  ...
  const doConvert = () => {
    ...
    toast.error(t('admin.crm.noRateAvailable'));
  };
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('admin.crm.exchangeRates')}</h1>
            <p className="text-sm text-gray-500">{t('admin.crm.exchangeRatesDesc')}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={syncRates} disabled={syncing} className="flex items-center gap-1.5 px-3 py-2 text-sm border rounded-md hover:bg-gray-50 disabled:opacity-50">
            <RefreshCcw className={`h-4 w-4 ${syncing ? 'animate-spin' : ''}`} /> {syncing ? 'Syncing...' : (t('admin.crm.syncFromApi'))}
          </button>
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-amber-600 text-white rounded-md hover:bg-amber-700">
            <Plus className="h-4 w-4" /> {t('admin.crm.addRate')}
          </button>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b">
                <th className="text-start text-xs font-medium text-gray-500 px-4 py-3">{t('common.from')}</th>
                <th className="text-start text-xs font-medium text-gray-500 px-4 py-3">{t('common.to')}</th>
                <th className="text-end text-xs font-medium text-gray-500 px-4 py-3">{t('admin.crm.rate')}</th>
                <th className="text-start text-xs font-medium text-gray-500 px-4 py-3">{t('common.source')}</th>
                <th className="text-start text-xs font-medium text-gray-500 px-4 py-3">{t('admin.crm.lastUpdated')}</th>
              </tr>
            </thead>
              ...
              {rates.map(rate => (
                <tr key={rate.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${rate.source === 'api' ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-600'}`}>{rate.source}</span>
                  </td>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 space-y-4" role="dialog" aria-modal="true" aria-labelledby="add-rate-modal-title">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">{t('admin.crm.rate')}</label>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowAdd(false)} className="px-4 py-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200">{t('common.cancel')}</button>
