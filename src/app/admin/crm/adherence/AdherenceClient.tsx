...
export default function AdherenceClient() {
  const { t } = useI18n();
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            {t('admin.crm.adherence.title')}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {t('admin.crm.adherence.subtitle')}
          </p>
        </div>
        {tab === 'realtime' && (
          <button
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-6 w-fit">
        ...
        >
          <span className="flex items-center gap-1.5">
            ...
            {t('admin.crm.adherence.realtime')}
          </span>
        </button>
        <button
        ...
        >
          <span className="flex items-center gap-1.5">
            ...
            {t('admin.crm.adherence.historical')}
          </span>
        </button>
      </div>
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="bg-[var(--k-glass-thin)] rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">
                {t('admin.crm.adherence.noAgentsDesc')}
              </p>
            </div>
          ) : (
                ...
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-start px-4 py-3 text-xs font-semibold text-gray-500 uppercase">
                      {t('admin.crm.adherence.agent')}
                    </th>
                    <th className="text-start px-4 py-3 text-xs font-semibold text-gray-500 uppercase">
                      {t('admin.crm.adherence.scheduled')}
                    </th>
                    <th className="text-start px-4 py-3 text-xs font-semibold text-gray-500 uppercase">
                      {t('admin.crm.adherence.status')}
                    </th>
                    <th className="text-start px-4 py-3 text-xs font-semibold text-gray-500 uppercase">
                      {t('admin.crm.adherence.adherence')}
                    </th>
                    <th className="text-start px-4 py-3 text-xs font-semibold text-gray-500 uppercase">
                      {t('admin.crm.adherence.reason')}
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={agent.agentId} className="border-b border-gray-100 hover:bg-gray-50/50">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {agent.agentImage ? (
                            <Image src={agent.agentImage} alt={agent.agentName || 'Agent avatar'} width={40} height={40} className="h-7 w-7 rounded-full" />
                          ) : (
