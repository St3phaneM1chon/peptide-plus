...
const STATUS_COLORS: Record<string, string> = {
  NEW: 'bg-indigo-100 text-indigo-700', CONTACTED: 'bg-yellow-100 text-yellow-700',
        ...
        <div className="flex gap-2">
          ...
          {lead.status !== 'CONVERTED' && (
            <button onClick={() => setShowConvert(true)} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-purple-600 text-white rounded-md hover:bg-purple-700">
              <Target className="h-4 w-4" /> {t('admin.crm.convertToDeal')}
            </button>
          )}
        </div>
      </div>
              ...
              <div className="space-y-2">
                {lead.tasks.map(task => (
                  <div key={task.id} className="flex items-center gap-3 p-2 rounded bg-gray-50">
                    <div className={`w-2 h-2 rounded-full ${task.status === 'COMPLETED' ? 'bg-green-500' : task.priority === 'URGENT' ? 'bg-red-500' : 'bg-indigo-500'}`} />
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border p-4 space-y-2.5">
            ...
            {lead.lastContactedAt && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('admin.crm.lastContact')}</span>
                <span>{new Date(lead.lastContactedAt).toLocaleDateString(locale)}</span>
              </div>
            )}
            {lead.nextFollowUpAt && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('admin.crm.nextFollowUp')}</span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button onClick={onClose} className="px-4 py-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200">{t('common.cancel')}</button>
