...
export default function AgentsClient({ agents: initial }: { agents: AgentMetrics[] }) {
  ...
  const SortHeader = ({ field, label }: { field: SortField; label: string }) => (
    <th
      className="px-3 py-3 text-start font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:text-indigo-600 select-none"
      onClick={() => handleSort(field)}
    >
      <div className="flex items-center gap-1">
        <span className="text-xs">{label}</span>
        <ArrowUpDown className={`w-3 h-3 ${sortField === field ? 'text-indigo-600' : 'text-gray-400'}`} />
      </div>
    </th>
  );
