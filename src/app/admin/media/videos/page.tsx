  ...
  return (
    <div className="p-6 max-w-5xl space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[var(--k-text-primary)]">{t('admin.media.videosTitle')}</h1>
        <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-[var(--k-glass-thin)] rounded-lg border border-[var(--k-border-subtle)] p-4 space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <button type="submit" disabled={saving} className="ms-auto px-4 py-2 bg-gradient-to-r from-[#6366f1] to-[#818cf8] text-white rounded text-sm hover:bg-indigo-700 disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {videos.map(v => {
            ...
            return (
              <div key={v.id} className="bg-[var(--k-glass-thin)] rounded-lg border transition-colors overflow-hidden ${selectedIds.has(v.id) ? 'border-indigo-400 ring-2 ring-indigo-200' : 'border-[var(--k-border-subtle)] hover:border-slate-300'}">
                <div className={`flex items-center gap-3 p-3 ${selectedIds.has(v.id) ? 'border-indigo-400 ring-2 ring-indigo-200' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-[var(--k-text-primary)] truncate">{v.title}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-lg" title={`${v.views} ${t('admin.media.videoViews')}`}>
                    ...
                    <form onSubmit={handleUpdate} className="space-y-3">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="ms-auto flex items-center gap-2">
                          <button type="button" onClick={() => setEditingId(null)} className="px-3 py-2 text-slate-600 border border-slate-300 rounded text-sm hover:bg-white/5">
                            {t('common.cancel')}
                          </button>
                          <button type="submit" disabled={updating} className="px-4 py-2 bg-gradient-to-r from-[#6366f1] to-[#818cf8] text-white rounded text-sm hover:bg-indigo-700 disabled:opacity-50">
                ...
                {previewVideoId === v.id && embedUrl && (
                  <div className="border-t border-[var(--k-border-subtle)] p-3 bg-white/5">
      ...
      {playingVideo && playingVideo.videoUrl && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6" onClick={() => setPlayingVideo(null)}>
          <div className="relative max-w-4xl w-full bg-black rounded-xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <button onClick={() => setPlayingVideo(null)} className="absolute top-3 end-3 p-1.5 bg-white/80 rounded-full hover:bg-[var(--k-glass-thin)] z-10" aria-label="Close video">
              <X className="w-5 h-5" />
            </button>
