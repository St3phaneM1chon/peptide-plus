...
import { Calendar, Send, Clock, Instagram, Facebook, Twitter, Plus, Image, Sparkles, Trash2 } from 'lucide-react';
...
export default function SocialSchedulerPage() {
  ...
  const generateCaption = () => {
    ...
    toast.success('Légende générée par IA');
  };

  const createPost = () => {
    if (!newPost.content || !newPost.scheduledAt) {
      toast.error('Contenu et date requis');
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">Planifiez vos publications sur les réseaux sociaux</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Planifiées', count: posts.filter(p => p.status === 'scheduled').length, color: 'text-blue-600' },
          { label: 'Brouillons', count: posts.filter(p => p.status === 'draft').length, color: 'text-slate-600' },
          { label: 'Publiées', count: posts.filter(p => p.status === 'published').length, color: 'text-green-600' },
          { label: 'Échouées', count: posts.filter(p => p.status === 'failed').length, color: 'text-red-600' },
        ].map(s => (
      ...
      {showComposer && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h3 className="font-semibold text-slate-800 mb-4">Nouvelle publication</h3>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={generateCaption} className="flex items-center gap-1 px-3 py-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 text-sm font-medium">
                <Sparkles className="w-4 h-4" /> IA Légende
              </button>
            ...
            <div key={post.id} className="bg-white rounded-xl border border-slate-200 p-5 flex items-start gap-4">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {post.status !== 'published' && (
                  <button onClick={() => publishNow(post.id)} className="p-2 text-slate-400 hover:text-green-600 hover:bg-green-50 rounded-lg" title="Publier maintenant">
                    <Send className="w-4 h-4" />
                  </button>
                )}
                <button onClick={() => deletePost(post.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Supprimer">
