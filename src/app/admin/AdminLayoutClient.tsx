...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 start-0 z-50 h-full bg-slate-900 text-white transition-all duration-300
                  ...
                  {section.items.map((item) => (
                    ...
                    >
                      ...
                      {item.badge && sidebarOpen && pendingOrders > 0 && (
                        <span className="ms-auto bg-red-500 text-white text-[10px] font-semibold px-1.5 py-0.5 rounded-full leading-none">
                          {pendingOrders}
                        </span>
                      )}
                    </Link>
      ...
      {/* Main content */}
      <div className={`transition-all duration-300 ${sidebarOpen ? 'lg:ms-60' : 'lg:ms-[68px]'}`}>
        {/* Top bar */}
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {pendingOrders > 0 && (
                <span className="absolute -top-0.5 -end-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-semibold rounded-full flex items-center justify-center leading-none px-1">
                  {pendingOrders}
                </span>
              )}
            </Link>
            ...
            >
              ...
              {unreadChats > 0 && (
                <span className="absolute -top-0.5 -end-0.5 min-w-[18px] h-[18px] bg-sky-500 text-white text-[10px] font-semibold rounded-full flex items-center justify-center leading-none px-1">
                  {unreadChats}
                </span>
              )}
            </Link>
