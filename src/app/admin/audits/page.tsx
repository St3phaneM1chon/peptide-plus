...
const severityColors: Record<string, { bg: string; text: string; border: string }> = {
  ...
  LOW: { bg: 'bg-indigo-50', text: 'text-indigo-700', border: 'border-indigo-200' },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        ...
        <StatBox label={isFr ? 'Faible' : 'Low'} value={stats.low} color="bg-indigo-100" />
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Link
          ...
          className="ms-auto text-sm text-indigo-600 hover:text-indigo-800 font-medium"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((at) => {
          ...
          return (
            <div
              key={at.id}
              className={`rounded-lg border ${colors.border} ${colors.bg} p-4 transition-shadow hover:shadow-md ${isRunning ? 'ring-2 ring-indigo-400' : ''}`}
            >
              {/* Header */}
