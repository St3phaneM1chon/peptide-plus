...
export default function CampaignList({ onEditCampaign }: CampaignListProps) {
  ...
  const statusConfig: Record<string, { label: string; color: string }> = {
    ...
    SCHEDULED: { label: t('admin.emails.campaigns.statusScheduled'), color: 'bg-indigo-100 text-indigo-700' },
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-32" role="status" aria-label="Loading"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-indigo-500" /><span className="sr-only">Loading...</span></div>;
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={creating}
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-indigo-500 hover:bg-indigo-600 rounded-lg disabled:opacity-50"
        >
      ...
      <ConfirmDialog
        ...
        title={t('admin.emails.campaigns.deleteTitle')}
