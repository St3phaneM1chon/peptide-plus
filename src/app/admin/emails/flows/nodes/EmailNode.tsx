...
export default function EmailNode({ data }: NodeProps) {
  ...
  return (
    <div className="bg-indigo-50 border-2 border-indigo-300 rounded-lg p-3 min-w-[180px] shadow-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-[10px] font-medium text-indigo-500 uppercase">{t('admin.emails.nodes.email')}</div>
          <div className="text-sm font-semibold text-indigo-900">{data.label}</div>
        </div>
      </div>
      {data.subject && (
        <div className="mt-2 text-[10px] text-indigo-600 truncate max-w-[160px]">
