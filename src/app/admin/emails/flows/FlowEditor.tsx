...
export default function FlowEditor({ flowId, onBack }: FlowEditorProps) {
  ...
  const NODE_TEMPLATES = [
    ...
    { type: 'email', label: t('admin.emails.flows.nodeEmail'), icon: Mail, color: 'bg-indigo-100 text-indigo-700' },
    ...
    <div className="flex flex-col h-[calc(100vh-200px)] bg-[var(--k-glass-thin)] rounded-xl border border-[var(--k-border-subtle)] overflow-hidden">
      ...
      <div className="flex items-center gap-3 p-3 border-b border-[var(--k-border-subtle)] bg-white/5">
        ...
        <button
          onClick={saveFlow}
          disabled={saving}
          className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-medium text-white bg-indigo-500 hover:bg-indigo-600 rounded-lg disabled:opacity-50"
        >
