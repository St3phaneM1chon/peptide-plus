...
export default function TemplateBuilder({ templateId, initialBlocks, onSaved }: TemplateBuilderProps) {
  ...
  const renderBlockPreview = (block: TemplateBlock) => {
    switch (block.type) {
      ...
      case 'image':
        return block.content.url ? <NextImage src={block.content.url} alt={block.content.alt || ''} width={200} height={200} style={{ maxWidth: '100%', height: 'auto' }} /> : <div className="h-32 bg-slate-100 rounded flex items-center justify-center text-slate-400">Image placeholder</div>;
      case 'product_grid':
          ...
          <div className="max-w-[600px] mx-auto bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
            {blocks.length === 0 ? (
              ...
              blocks.map((block) => (
                <div
                  key={block.id}
                  className={`relative group ${selectedBlock === block.id ? 'ring-2 ring-indigo-400' : ''} ${!previewMode ? 'hover:ring-1 hover:ring-slate-300 cursor-pointer' : ''}`}
