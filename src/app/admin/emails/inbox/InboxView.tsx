          ...
          >
            ...
            {s.count > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${
                statusFilter === s.key ? 'bg-indigo-200' : 'bg-slate-200'
              }`}>
                {s.count}
              </span>
            )}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center">
                    {conv.customer?.image ? (
                      <Image src={conv.customer.image} alt={conv.customer.name || 'Customer avatar'} className="w-8 h-8 rounded-full" width={32} height={32} />
                    ) : (
