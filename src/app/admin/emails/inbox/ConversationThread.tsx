  ...
  if (!conversation) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <button onClick={onBack} className="text-indigo-500 text-sm mt-2">{t('admin.emails.inbox.back')}</button>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <select
                ...
                className={`text-xs font-medium px-2 py-1 rounded border-0 cursor-pointer ${
                  conversation.status === 'NEW' ? 'bg-indigo-100 text-indigo-700' :
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
          {timeline.map((item) => (
            <div key={`${item.type}-${item.id}`}>
              ...
              {item.type === 'outbound' && (
                <div className="bg-indigo-50 rounded-lg shadow-sm border border-indigo-200 p-4 ms-8">
                  <div className="flex items-center gap-2 mb-2">
          ...
          {replyMode ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-xs font-medium px-2 py-0.5 rounded ${
                  replyMode === 'reply' ? 'bg-indigo-100 text-indigo-700' : 'bg-yellow-100 text-yellow-700'
                }`}>
              ...
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium text-white rounded-lg disabled:opacity-50 ${
                    replyMode === 'note' ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-indigo-500 hover:bg-indigo-600'
                  }`}
                >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium text-white bg-indigo-500 hover:bg-indigo-600 rounded-lg"
              >
