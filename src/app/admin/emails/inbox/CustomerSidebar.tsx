...
export default function CustomerSidebar({ customer, stats }: CustomerSidebarProps) {
  ...
  return (
    <div className="w-72 border-l border-[var(--k-border-subtle)] bg-[var(--k-glass-thin)] overflow-y-auto">
      {/* Customer header */}
      <div className="p-4 border-b border-slate-100">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-12 h-12 rounded-full bg-slate-200 flex items-center justify-center">
            {customer.image ? (
              <Image src={customer.image as string} alt={(customer.name as string) || 'Customer avatar'} className="w-12 h-12 rounded-full" width={48} height={48} />
            ) : (
              <User className="h-6 w-6 text-slate-400" />
            )}
          </div>
          <div>
            <h3 className="font-semibold text-[var(--k-text-primary)]">{(customer.name as string) || t('admin.emails.inbox.customer')}</h3>
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-100 border-b border-slate-100">
          <div className="bg-[var(--k-glass-thin)] p-3 text-center">
            <ShoppingBag className="h-4 w-4 mx-auto text-slate-400 mb-1" />
            <div className="text-lg font-semibold text-[var(--k-text-primary)]">{stats.orderCount as number}</div>
          ...
          <div className="bg-[var(--k-glass-thin)] p-3 text-center">
            ...
            <div className="text-lg font-semibold text-[var(--k-text-primary)]">
          ...
          <div className="bg-[var(--k-glass-thin)] p-3 text-center">
            ...
            <div className="text-lg font-semibold text-[var(--k-text-primary)]">{stats.conversationCount as number}</div>
          ...
          <div className="space-y-2">
            {orders.map((order) => (
              <div key={order.id as string} className="flex items-center justify-between text-sm">
                <div>
                  <span className="font-medium text-[var(--k-text-primary)]">#{order.orderNumber as string}</span>
