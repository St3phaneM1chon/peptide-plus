...
export default function EmailComposer({ onClose, replyTo }: EmailComposerProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--k-glass-thin)]">
      {/* Toolbar */}
      <div className="flex items-center gap-2 px-4 py-2 border-b border-[var(--k-border-subtle)] bg-white/5 flex-shrink-0">
      ...
      {/* Fields */}
      <div className="px-4 py-2 space-y-2 border-b border-[var(--k-border-subtle)] flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          />
          {!showCc && (
            <button type="button" onClick={() => setShowCc(true)} className="text-xs text-indigo-600 hover:underline">
              Cc
            </button>
          )}
        </div>
      ...
      {attachments.length > 0 && (
        <div className="px-4 py-2 border-b border-slate-100 flex-shrink-0 flex flex-wrap gap-2">
          {attachments.map((att) => (
            <div
              key={att.filename}
              className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 border border-[var(--k-border-subtle)] rounded-lg text-xs text-slate-700 max-w-[200px]"
            >
      ...
      <div
        ...
        aria-multiline="true"
        className="flex-1 px-4 py-3 text-sm text-[var(--k-text-primary)] overflow-y-auto focus:outline-none"
