...
function SignInContent() {
  ...
  const handleCredentialsSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Role-based redirect: OWNER/EMPLOYEE/CLIENT → admin, CUSTOMER → homepage
      ...
      <div className="max-w-md w-full">
        ...
        {registered && (
          <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg text-green-700">
            <p className="text-sm">{t('auth.registeredSuccess')}</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          {/* Boutons OAuth - order: Apple, Microsoft, Google, X, LinkedIn */}
          <div className="space-y-3 mb-6">
            ...
            {(!providersLoaded || 'microsoft-entra-id' in availableProviders) && (
            <button
            ...
            {providersLoaded && 'linkedin' in availableProviders && (
            <button
              onClick={() => handleOAuthSignIn('linkedin')}
              disabled={isLoading}
              className="w-full flex items-center justify-center px-4 py-3 border-2 border-gray-200 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50"
            >
              <svg className="w-5 h-5 me-3" fill="#0A66C2" viewBox="0 0 24 24">
