...
function StudyTimer({ t }: { t: (k: string, p?: Record<string, string | number>) => string }) {
 ...
 className="p-1 rounded hover:bg-[var(--k-glass-thin)] transition-colors"
 ...
 >
 {running ? (
 <svg className="w-4 h-4 text-[var(--k-text-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <span className="font-mono text-[var(--k-text-secondary)] tabular-nums" aria-live="off">
 {fmt(seconds)}
 </span>
 </div>
 );
...
function NotesPanel({
 ...
 return (
 <div className="bg-[var(--k-glass-regular)] backdrop-blur-xl border border-[var(--k-border-subtle)] rounded-xl shadow-lg p-4 w-full">
 <div className="flex items-center justify-between mb-3">
 <h3 className="font-semibold text-[var(--k-text-primary)] text-sm flex items-center gap-2">
 ...
 <button
 onClick={handleSave}
 className="text-xs px-2 py-1 bg-blue-50 text-[var(--k-accent-indigo)] rounded hover:bg-blue-100 transition-colors"
 >
 {t('learn.lessonViewer.saveNotes')}
 </button>
 <button onClick={onClose} className="text-[var(--k-text-tertiary)] hover:text-[var(--k-text-secondary)]" aria-label={t('learn.lessonViewer.closeNotes')}>
...
export default function LessonViewerClient({
 ...
 return md
 .replace(/^### (.+)$/gm, '<h3 class="text-lg font-semibold text-[var(--k-text-primary)] mt-6 mb-2">$1</h3>')
 .replace(/^## (.+)$/gm, '<h2 class="text-xl font-bold text-[var(--k-text-primary)] mt-8 mb-3">$1</h2>')
 .replace(/^# (.+)$/gm, '<h1 class="text-2xl font-bold text-[var(--k-text-primary)] mt-8 mb-4">$1</h1>')
 .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
 .replace(/\*(.+?)\*/g, '<em>$1</em>')
 .replace(/`(.+?)`/g, '<code class="bg-[var(--k-glass-thin)] px-1 py-0.5 rounded text-sm">$1</code>')
 .replace(/^- (.+)$/gm, '<li class="ml-4 list-disc text-[var(--k-text-secondary)]">$1</li>')
 .replace(/^(\d+)\. (.+)$/gm, '<li class="ml-4 list-decimal text-[var(--k-text-secondary)]">$2</li>')
 .replace(/\n\n/g, '</p><p class="text-[var(--k-text-secondary)] leading-relaxed mb-4">')
 .replace(/^/, '<p class="text-[var(--k-text-secondary)] leading-relaxed mb-4">')
 .replace(/$/, '</p>');
 };
 ...
 className="lg:hidden p-1.5 rounded-lg hover:bg-[var(--k-glass-thin)] transition-colors"
 aria-label={t('learn.lessonViewer.toggleOutline')}
 >
 <svg className="w-5 h-5 text-[var(--k-text-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
 </svg>
 </button>
 <Link
 href={`/learn/${courseSlug}`}
 className="flex items-center gap-1 text-[var(--k-text-secondary)] hover:text-[var(--k-text-primary)] text-sm flex-shrink-0"
 >
 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <span className="text-[var(--k-text-tertiary)] hidden sm:inline">|</span>
 <span className="text-sm text-[var(--k-text-secondary)] truncate hidden sm:inline">{courseTitle}</span>
 </div>

 <div className="flex items-center gap-4">
 {/* Course Progress Bar */}
 <div className="hidden md:flex items-center gap-2">
 <div className="w-32 bg-[var(--k-glass-thin)] rounded-full h-1.5">
 <div
 className="bg-green-500 h-1.5 rounded-full transition-all duration-500"
 style={{ width: `${courseProgress}%` }}
 />
 </div>
 <span className="text-xs text-[var(--k-text-secondary)] whitespace-nowrap">{courseProgress}%</span>
 </div>
 ...
 {/* Lesson counter */}
 <span className="text-sm text-[var(--k-text-tertiary)] flex-shrink-0">
 ...
 <aside
 className={`fixed lg:sticky top-[49px] lg:top-[49px] left-0 z-20 h-[calc(100vh-49px)] w-72 bg-[var(--k-bg-surface)] border-r border-[var(--k-border-subtle)] overflow-y-auto transition-transform lg:transition-none lg:translate-x-0 ${
 sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
 }`}
 >
 <div className="p-4 border-b border-[var(--k-border-subtle)]">
 <h2 className="font-semibold text-[var(--k-text-primary)] text-sm truncate">{courseTitle}</h2>
 ...
 <span className="text-xs text-[var(--k-text-secondary)]">{courseProgress}%</span>
 </div>
 </div>
 ...
 return (
 <div key={ch.id}>
 <div className="px-4 py-2 text-xs font-semibold text-[var(--k-text-tertiary)] uppercase tracking-wider">
 {ci + 1}. {ch.title}
 </div>
 <ul>
 {ch.lessons.map((ol) => {
 ...
 return (
 <li key={ol.id}>
 <div className="flex items-center gap-2 px-4 py-2 text-sm text-[var(--k-text-tertiary)] cursor-not-allowed" title="Completez la lecon precedente d'abord">
 ...
 isCurrent
 ? 'bg-[var(--k-accent-indigo-10)] text-[var(--k-accent-indigo)] font-medium border-l-2 border-[var(--k-accent-indigo)]'
 : isDone
 ...
 <p className="text-sm text-[var(--k-accent-indigo)] font-medium">
 {t('learn.lessonViewer.chapterProgress', { chapter: chapter.title })}
 </p>
 {readingTime && (
 <span className="text-xs text-[var(--k-text-tertiary)] flex items-center gap-1">
 ...
 <h1 className="text-2xl font-bold text-[var(--k-text-primary)]">{lesson.title}</h1>
 {lesson.description && (
 <p className="text-[var(--k-text-secondary)] mt-1">{lesson.description}</p>
 )}
 </div>
 ...
 : 'border-gray-200 text-[var(--k-text-secondary)] hover:bg-[var(--k-glass-thin)]'
 }`}
 >
 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
 ...
 <input
 type="text"
 className="flex-1 px-3 py-2 border border-purple-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 bg-[var(--k-glass-thin)]"
 ...
 {/* Content Area */}
 <div className="bg-[var(--k-glass-regular)] backdrop-blur-xl rounded-xl border border-[var(--k-border-subtle)] shadow-[var(--k-shadow-md)] border border-gray-200 overflow-hidden mb-8">
 ...
 <div className="p-6 md:p-8 space-y-6 border-t border-[var(--k-border-subtle)]">
 {/* Volet 1: Texte du manuel */}
 {lesson.manualText && (
 <div>
 <h3 className="text-sm font-semibold uppercase tracking-wide text-[var(--k-accent-indigo)] mb-3 flex items-center gap-2">
 ...
 <h2 className="text-xl font-bold text-[var(--k-text-primary)] mb-2">{t('learn.lessonViewer.quizLesson')}</h2>
 <p className="text-[var(--k-text-secondary)] mb-6">{lesson.description || t('lms.quiz.title')}</p>
 <Link
 ...
 <div className="p-6 md:p-8 text-center text-[var(--k-text-secondary)]">
 ...
 <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[var(--k-glass-regular)] backdrop-blur-xl rounded-xl border border-[var(--k-border-subtle)] p-4">
 {/* Mark Complete */}
 <div>
 {completed ? (
 ...
 className="inline-flex items-center gap-1 px-4 py-2.5 border border-[var(--k-border-default)] text-[var(--k-text-secondary)] font-medium rounded-lg hover:bg-[var(--k-glass-ultra-thin)] transition-colors text-sm"
 >
 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
 </svg>
 ...
 className="inline-flex items-center gap-1 px-4 py-2.5 bg-gradient-to-r from-[#6366f1] to-[#818cf8] text-white font-medium rounded-lg hover:from-[#5558e6] hover:to-[#737de6] transition-colors text-sm"
 >
